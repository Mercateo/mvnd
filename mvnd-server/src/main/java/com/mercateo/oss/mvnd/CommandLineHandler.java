...
public class CommandLineHandler implements Runnable {
    ...
    public void run() {
        ...
        assume(mvnDir.isDirectory(), mvnDir.getAbsolutePath()
                + " does not exist, or is not a directory.");
        assume(mvn.exists(), mvn.getAbsolutePath() + " does not exist.");
        assume(!conf.exists(), "Skipping installation. (" + conf.getAbsolutePath()
                + " already there)");
    ...
    private void createMvnc(File mvnc) {
        ...
        String graalClient = "/client/" + os + "/mvnc";
        client = MVNDaemon.class.getResource(graalClient);
        if (client == null) {
            System.out.println("Graal based client not available - mvnd was buildt without Graal.");

            String goClient = "/client/" + os + "/mvn-go";
            client = MVNDaemon.class.getResource(goClient);
            if (client == null)
            {
                System.out.println("Go based client not available.");
                return; }
        }

        try (InputStream is = client.openStream()) {
            if (is == null)
    ...
    private void noClientAvailableError() {
        ...
        System.out.println();
        System.out.println(
                "There is no prebuilt client for you environment, so you should head over to");
        System.out.println("https://github.com/uweschaefer/mvn-go");
        System.out.println("and build it yourself.");
        System.out.println();
        System.out.println(
                "If you have any problems, or (even better) want to volunteer, create an issue on github");
        System.out.println();
    ...
    private void createLibMvndJar(File lib) {
        ...
        assume(url.toExternalForm().endsWith(".jar"),
                "Was assuming to be run from a jar... exiting.");
    ...
    private void createBinM2dConf(File bin) {
        ...
        Unix4j.fromFile(new File(bin, "m2.conf"))
                .sed("s/org.apache.maven.cli.MavenCli/com.mercateo.oss.mvnd.MVNDaemon/g")
                .toFile(m2d);
    ...
    void help(boolean full) {
        ...
        if (full) {
            System.out.println("");
            System.out.println(
                    " java -jar " + MVNDaemon.class.getProtectionDomain()
                            .getCodeSource()
                            .getLocation()
                            .getFile()
